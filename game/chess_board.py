...
class GameBoard:  # или игра?
    def __init__(self, new_game=True, board_str=None, current=None):
        ...
        if new_game:
            ...
            self.opponent_color = 'white' if self.current == 'black' else 'black'
        ...
        for figure in board_list:
            ...
            if figure["cls"] == 'King':
                if figure["side"] == 'white':
                    ...
                    self.kb = [row, col]
        self.calc_attack_map(side=self.opponent_color)
        result = self.calc_attack_map(side=self.current)
        print(result)
        if result == 'check': 
            self.is_check(self.current)
            print(self.board[self.kw[0]][self.kw[1]].moves)
